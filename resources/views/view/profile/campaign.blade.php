            ...
            <div class="col-md-3">
                <nav class="nav-profile">
                    <ul class="list-profile">
                        ...
                        <li class="">
                            <a class="active" href="javascript:void(0)">Chiến dịch đã tạo</a>
                        </li>
                        <li class="">
                            <a class="" href="{{route('view.profile-articles', $id)}}">Tin tức đã tạo</a>
                        </li>
                    </ul>
                </nav>
            </div>
                    ...
                    <li class="post-item d-flex align-items-center border mb-4 p-3">
                        <div class="post-avt w-25">
                            <a href="{{route('view.campaign.detail', $item->id)}}">
                                <img src="{{asset($item->image ?? 'images/no-img.jpg')}}" class="w-100" alt="image">
                            </a>
                        </div>
                        <div class="post-content w-75 ml-3">
                            <div class="post-title">
                                <a href="{{route('view.campaign.detail', $item->id)}}" class="h6 text-primary">
                                    {{$item->name}}
                                </a>
                            </div>
                            <div class="post-des">
                            ...
                            <div class="dropdown text-right">
                                ...
                                <div class="dropdown-menu text-center min-w-0">
                                    <a class="dropdown-item" href="{{route('view.campaign.detail', $item->id)}}">Xem</a>
                                    <a class="dropdown-item" href="{{route('view.campaign.edit', $item->id)}}">Sửa</a>
                                    <a class="dropdown-item del-item" href="javascript:void(0);" data-form="#form-{{$item->id}}">Xóa</a>
                                    <form action="{{route('view.delete-campaign', $item->id)}}" id="form-{{$item->id}}" method="POST">
                                        @csrf
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="post-link text-right">
                                <a class="text-sm" href="{{route('view.campaign.detail', $item->id)}}"><small>Xem thêm</small></a>
                            </div>
                            @endif
                        </div>
                    </li>
